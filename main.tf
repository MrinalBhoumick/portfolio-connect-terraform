...
resource "aws_lb_listener" "http" {
  load_balancer_arn = aws_alb.HandsOnELB.arn
  port              = "80"
  protocol          = "HTTP"
...
resource "aws_security_group" "ec2_sg" {
  name        = "allow_http_ec2"
  description = "Allow http - https inbound traffic for elb"
  vpc_id      = aws_vpc.CustomVPC.id
...
resource "aws_autoscaling_group" "autoscaling_group_webteir" {
  ...
  tag {
    key                 = "Name"
    value               = "autoscaling-group-webtier"
